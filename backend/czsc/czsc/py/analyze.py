...
def check_bi(bars: List[NewBar], **kwargs):
    ...
    # 新笔：缺口（跳空）能量豁免最小成笔长度
    # 在笔覆盖区间 bars_a 中，若相邻两根无包含K线存在明显缺口，则允许把最小成笔长度降到 4。
    # 仅检查 bars_a 的“第二根到倒数第二根”区间（排除首尾两根）；并使用该区间对应的原始K线（raw_bars）检查缺口
    _mid_bars = [rb for nb in bars_a[1:-1] for rb in nb.raw_bars]
    has_gap = any(
        (k1.high < k2.low) or (k1.low > k2.high) for k1, k2 in zip(_mid_bars, _mid_bars[1:])
    )
    min_bi_len_ = 4 if has_gap and min_bi_len > 4 else min_bi_len

    # 成笔的条件：1）顶底分型之间没有包含关系；2）笔长度大于等于min_bi_len
    if (not ab_include) and (len(bars_a) >= min_bi_len_):
