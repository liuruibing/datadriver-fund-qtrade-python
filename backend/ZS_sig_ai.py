...
def calculate_xd_list(bis: List[BI]) -> List[Dict]:
    """基于结构转折的缠论线段划分方案
    抛弃了硬编码固定步长，采用高低点破坏原则划分线段。
    规则：
    1. 同向高点/低点持续延伸则线段延续。
    2. 当出现反方向突破且回调不再创新高/新低，确立线段转折。
    """
    if len(bis) < 3:
        return []
    ...
    # 寻找结构性转折点
    xd_start_idx = 0
    xd_dir = bis[0].direction
    
    for i in range(2, len(bis)):
        # 如果当前线段向上，寻找结束标志（向下突破前低且反弹不破前高）
        if xd_dir == Direction.Up:
            if bis[i].direction == Direction.Down and i >= xd_start_idx + 2:
                # 创出结构性新低，疑似线段转折
                # 采用就近转折点确认：H2 < H1 且 L2 < L1
                if bis[i].low < bis[i-2].low:
                    # 如果后续笔无法突破前高，则确认转折
                    if i + 1 < len(bis) and bis[i+1].high < bis[i-1].high:
                        seg_bis = bis[xd_start_idx:i]
                        xds.append({
                            "start_dt": seg_bis[0].sdt,
                            "end_dt": seg_bis[-1].edt,
                            "direction": "向上",
                            "start_val": round(min(b.low for b in seg_bis), 2),
                            "end_val": round(max(b.high for b in seg_bis), 2),
                            "completed": True,
                            "bis_count": len(seg_bis),
                            "bi_list": seg_bis,
                            "beichi": ""
                        })
                        xd_dir = Direction.Down
                        xd_start_idx = i
                        
        # 如果当前线段向下，寻找结束标志（向上突破前高且回调不破前低）
        elif xd_dir == Direction.Down:
            if bis[i].direction == Direction.Up and i >= xd_start_idx + 2:
                # 采用就近转折点确认：L2 > L1 且 H2 > H1
                if bis[i].high > bis[i-2].high:
                    if i + 1 < len(bis) and bis[i+1].low > bis[i-1].low:
                        seg_bis = bis[xd_start_idx:i]
                        xds.append({
                            "start_dt": seg_bis[0].sdt,
                            "end_dt": seg_bis[-1].edt,
                            "direction": "向下",
                            "start_val": round(max(b.high for b in seg_bis), 2),
                            "end_val": round(min(b.low for b in seg_bis), 2),
                            "completed": True,
                            "bis_count": len(seg_bis),
                            "bi_list": seg_bis,
                            "beichi": ""
    ...
    # 处理最后一段（未完成或进行中）
    if xd_start_idx < len(bis):
        seg_bis = bis[xd_start_idx:]
        direction = "向上" if xd_dir == Direction.Up else "向下"

        if xd_dir == Direction.Up:
            # 向上线段：找到最高点所在的笔
            peak_bi = max(seg_bis, key=lambda b: b.high)
            peak_idx = seg_bis.index(peak_bi)
            start_val = min(b.low for b in seg_bis)
            end_val = peak_bi.high
            end_dt = peak_bi.edt
            # 如果最高点之后还有至少3笔（说明走势已经回落确认），则认为线段已完成
            remaining_after_peak = len(seg_bis) - peak_idx - 1
            completed = remaining_after_peak >= 3
        else:
            # 向下线段：找到最低点所在的笔
            trough_bi = min(seg_bis, key=lambda b: b.low)
            trough_idx = seg_bis.index(trough_bi)
            start_val = max(b.high for b in seg_bis)
            end_val = trough_bi.low
            end_dt = trough_bi.edt
            # 如果最低点之后还有至少3笔（说明走势已经反弹确认），则认为线段已完成
            remaining_after_trough = len(seg_bis) - trough_idx - 1
            completed = remaining_after_trough >= 3
    ...
    for idx, xd in enumerate(xds):
        if idx == 0 or not xd["completed"]:
            xd["beichi"] = ""
            continue
        prev_xd = None
        for pxd in reversed(xds[:idx]):
            if pxd["direction"] == xd["direction"]:
                prev_xd = pxd
                break
        if not prev_xd:
            xd["beichi"] = ""
        ...
        xd_range = abs(xd["end_val"] - xd["start_val"])
...
def _check_first_buy(bis: List[BI]) -> Dict:
    """检查 bis 是否是一买的结束 —— 对齐官方 cxt_first_buy_V221126

    :param bis: 笔序列，按时间升序
    :return: {"match": bool, "bi": 触发笔}
    """
    res = {"match": False, "bi": None}
    ...
    # 检查背驰：最后一笔的 power_price，power_volume，length
    pp_ref = max(bis[-3].power_price, np.mean([x.power_price for x in key_bis]))
    bc_price = bis[-1].power_price < pp_ref
    ...
    # 条件1: 价格背驰 + (量背驰 或 时间背驰)  —— 官方标准
    # 条件2: 价格力度增幅温和(< 30%) + 量背驰 + 时间背驰  —— "缩量创新低", 量价背离补充
    bc_price_mild = bis[-1].power_price < pp_ref * 1.3
    if (bc_price and (bc_volume or bc_length)) or (bc_price_mild and bc_volume and bc_length):
        res["match"] = True
        res["bi"] = bis[-1]
    return res


def _check_first_sell(bis: List[BI]) -> Dict:
    """检查 bis 是否是一卖的结束 —— 对齐官方 cxt_first_sell_V221126
    ...
    # 条件1: 价格背驰 + (量背驰 或 时间背驰)  —— 官方标准
    # 条件2: 价格力度增幅温和(< 30%) + 量背驰 + 时间背驰  —— "缩量创新高", 量价背离补充
    bc_price_mild = bis[-1].power_price < pp_ref * 1.3
    if (bc_price and (bc_volume or bc_length)) or (bc_price_mild and bc_volume and bc_length):
        res["match"] = True
        res["bi"] = bis[-1]
...
def find_B1(bi_list: List[BI], zs_list: List[ZS], czsc_obj: CZSC, strict: bool = True, th: int = 100) -> List[Dict]:
    """
    遍历检查第一类买点
    :param bi_list: 已完成的笔列表（按时间正序）
    :param zs_list: 已完成的中枢列表（按时间正序）
    :param czsc_obj: 缠论数据计算对象（用于计算MACD力度等）
    :param strict: 是否启用严格模式。True 为趋势背驰（需2中枢），False 为盘整背驰（1中枢即可）
    :param th: 背驰力度比较阈值
    :return: 买点列表
    """
    buy_points = []
    ...
    for i, current_bi in enumerate(bi_list):
        ...
        # 严格模式：至少需要两个中枢才能形成趋势；非严格模式：至少一个中枢
        min_zs_count = 2 if strict else 1
        if len(valid_zs_list) < min_zs_count:
            continue
        ...
        try:
            if strict:
                # 趋势背驰逻辑：A 与 B 的连接段 b vs B 的离开段 c
                if not check_down_trend(valid_zs_list):
                    continue
                prev_zs = valid_zs_list[-2]
                # b 段：从 A 的最后一笔开始，到 B 的第一笔开始
                b_sdt = prev_zs.bis[-1].sdt
                b_edt = last_zs.bis[0].sdt
            else:
                # 盘整背驰逻辑：进入中枢段 entry vs 离开中枢段 c
                entry_bi = get_entry_BI(last_zs, bi_list)
                if not entry_bi:
                    continue
                b_sdt = entry_bi.sdt
                b_edt = entry_bi.edt
        ...
        # c 段：从 B 中枢内，距离 current_bi 最近且同向的笔开始，到 current_bi 结束
        c_candidates = [
...
def find_S1(bi_list: List[BI], zs_list: List[ZS], czsc_obj: CZSC, strict: bool = True, th: int = 100) -> List[Dict]:
    """
    遍历检查第一类卖点
    :param bi_list: 已完成的笔列表（按时间正序）
    :param zs_list: 已完成的中枢列表（按时间正序）
...
def find_B2(bi_list: List[BI], b1_list: List[Dict], independent: bool = False) -> List[Dict]:
    """
    遍历检查第二类买点
    ...
    :param independent: 是否开启独立检测模式（不依赖一类买点）
    :return: 第二类买点列表
    """
    buy_points = []
    
    # 如果是独立检测模式，先寻找潜在的底部（不破低的向下笔）
    if independent:
        for i in range(3, len(bi_list)):
            bi = bi_list[i]
            if bi.direction == Direction.Down:
                # 寻找最近的一个向下笔
                prev_down = bi_list[i-2]
                if bi.low > prev_down.low:
                    # 检查中间是否有一个向上突破（可选可不选，缠论中 B2 必须在 B1 之后）
                    # 独立模式下，只要是不破前低的底分型且前方有一定跌幅，就标记
                    buy_points.append({
                        "dt": bi.edt,
                        "price": bi.low,
                        "op": Operate.LO,
...
def find_S2(bi_list: List[BI], s1_list: List[Dict], independent: bool = False) -> List[Dict]:
    """
    遍历检查第二类卖点
    ...
    :param independent: 是否开启独立检测模式（不依赖一类卖点）
    :return: 第二类卖点列表
    """
    sell_points = []
...
def find_LB2(bi_list: List[BI], b2_list: List[Dict]) -> List[Dict]:
    """类二买：在二买位置或之后，底分型抬升的向下笔。
    
    官方行为：类二买可以与二买出现在同一笔上（当该向下笔的 low 高于前一个同向向下笔的 low 时）。
    """
    res = []
    seen_dts = set()
    ...
    for b2 in b2_list:
        ...
        for i, bi in enumerate(bi_list):
            if bi.edt == b2_dt:
                # 模式1: B2 所在笔本身就是类二买（该向下笔的 low > 前一个同向向下笔的 low）
                if i >= 2 and bi.direction == Direction.Down:
                    prev_down_bi = bi_list[i - 2]
                    if prev_down_bi.direction == Direction.Down and bi.low > prev_down_bi.low:
                        dt_key = str(bi.edt)
                        if dt_key not in seen_dts:
                            seen_dts.add(dt_key)
                            res.append({"dt": bi.edt, "price": bi.low, "op": Operate.LO, "op_desc": "LB2", "bs_type": "LB2"})

                # 模式2: B2 之后的下一个向下笔也可能是类二买
                if i + 2 < len(bi_list):
                    bi3 = bi_list[i + 2]
                    if bi3.direction == Direction.Down and bi3.low > b2_price:
                        dt_key = str(bi3.edt)
                        if dt_key not in seen_dts:
                            seen_dts.add(dt_key)
                            res.append({"dt": bi3.edt, "price": bi3.low, "op": Operate.LO, "op_desc": "LB2", "bs_type": "LB2"})
                break

    return res

def find_bi_bc_signals(bi_list: List[BI]) -> List[Dict]:
    """
    纯笔背驰检测（不依赖中枢）
    如果当前笔相对于前一笔在价格动能（power_price）上表现出衰竭，即为潜在买卖点。
    """
    res = []
    if len(bi_list) < 5:
        return res
    ...
    for i in range(4, len(bi_list)):
        ...
        # 向下笔背驰（潜在买点）
        if curr.direction == Direction.Down and prev.direction == Direction.Down:
            if curr.low < prev.low: # 创新低
                # 价格力度背驰
                if curr.power_price < prev.power_price * 0.8:
                    res.append({
                        "dt": curr.edt,
                        "price": curr.low,
                        "op": Operate.LO,
        ...
        # 向上笔背驰（潜在卖点）
        if curr.direction == Direction.Up and prev.direction == Direction.Up:
            if curr.high > prev.high: # 创新高
                if curr.power_price < prev.power_price * 0.8:
                    res.append({
                        "dt": curr.edt,
                        "price": curr.high,
...
def find_LS2(bi_list: List[BI], s2_list: List[Dict]) -> List[Dict]:
    """类二卖：在二卖位置或之后，顶分型降低的向上笔。
    
    官方行为：类二卖可以与二卖出现在同一笔上（当该向上笔的 high 低于前一个同向向上笔的 high 时）。
    """
    res = []
    seen_dts = set()
    ...
    for s2 in s2_list:
        ...
        for i, bi in enumerate(bi_list):
            if bi.edt == s2_dt:
                # 模式1: S2 所在笔本身就是类二卖（该向上笔的 high < 前一个同向向上笔的 high）
                if i >= 2 and bi.direction == Direction.Up:
                    prev_up_bi = bi_list[i - 2]
                    if prev_up_bi.direction == Direction.Up and bi.high < prev_up_bi.high:
                        dt_key = str(bi.edt)
                        if dt_key not in seen_dts:
                            seen_dts.add(dt_key)
                            res.append({"dt": bi.edt, "price": bi.high, "op": Operate.LE, "op_desc": "LS2", "bs_type": "LS2"})

                # 模式2: S2 之后的下一个向上笔也可能是类二卖
                if i + 2 < len(bi_list):
                    bi3 = bi_list[i + 2]
                    if bi3.direction == Direction.Up and bi3.high < s2_price:
                        dt_key = str(bi3.edt)
                        if dt_key not in seen_dts:
                            seen_dts.add(dt_key)
                            res.append({"dt": bi3.edt, "price": bi3.high, "op": Operate.LE, "op_desc": "LS2", "bs_type": "LS2"})
                break
